...
type
  TMipMap2DApp = class(TApplication)
  private
    FProgram: TGLProgram;
...
procedure TMipMap2DApp.Initialize;
var
...
procedure TMipMap2DApp.KeyDown(const AKey: Integer; const AShift: TShiftState);
begin
...
procedure TMipMap2DApp.Render(const ADeltaTimeSec, ATotalTimeSec: Double);
type
  TVertex = record
    Pos: TVector4;
    TexCoord: TVector2;
...
const
  VERTICES: array [0..3] of TVertex = (
    (Pos: (X: -0.5; Y:  0.5; Z: 0.0; W: 5.0); TexCoord: (X: 0.0; Y: 0.0)),
    (Pos: (X: -0.5; Y: -0.5; Z: 0.0; W: 1.0); TexCoord: (X: 0.0; Y: 1.0)),
    (Pos: (X:  0.5; Y: -0.5; Z: 0.0; W: 1.0); TexCoord: (X: 1.0; Y: 1.0)),
    (Pos: (X:  0.5; Y:  0.5; Z: 0.0; W: 5.0); TexCoord: (X: 1.0; Y: 0.0)));
...
begin
  ...
  FTexture.MagFilter(TGLMagFilter.Nearest);
  FTexture.MinFilter(TGLMinFilter.Nearest);
  FUniOffset.SetValue(-0.6);
  gl.DrawElements(TGLPrimitiveType.Triangles, INDICES);

  { Draw quad with trilinear filtering }
  FTexture.MagFilter(TGLMagFilter.Linear);
  FTexture.MinFilter(TGLMinFilter.LinearMipmapLinear);
  FUniOffset.SetValue(0.6);
  gl.DrawElements(TGLPrimitiveType.Triangles, INDICES);
end;

procedure TMipMap2DApp.Shutdown;
begin
  { Release resources }
  FTexture.Delete;
  FProgram.Delete;
